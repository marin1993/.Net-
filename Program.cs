    ...
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            foreach (var item in apiInfo)
            {
                ...
                foreach (JsonNode uinfo in jsonNode["results"].AsArray())
                {
                    // 学完的用户
            ...
            Console.WriteLine($"完成 2 次挑战 {user.Where(x => x.learnNum ==2).Count()}人");
            Console.WriteLine($"完成 3 次挑战 {user.Where(x => x.learnNum == 3).Count()}人\n 分别是");
